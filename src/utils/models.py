...
def model_fit(model, train_dataset, epochs, batch_size, STEPS_PER_EPOCH, directory):
    call_backs = get_callbacks(directory)
    model.fit_generator(generator=train_dataset, steps_per_epoch=STEPS_PER_EPOCH, epochs=epochs, verbose=1) #, callbacks=call_backs)
